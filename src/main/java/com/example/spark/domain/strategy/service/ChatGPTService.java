...
public class ChatGPTService {
    ...
    public Map<String, Object> getGrowthStrategy(String activityDomain, String workType, String snsGoal, List<String> weaknesses, List<String> guides) {
        ...
        String prompt = """
                ...
                📌 **요청사항:**
                - 아래 제공된 `guides` 리스트 (Java 매개변수: `List<String> guides`)를 참고해  {{약점 지표1}}, {{약점 지표2}}를 개선할 수 있는 실행 비법 3가지를 작성합니다.
                ...
                4. 말투는 20대 여성 크리에이터처럼 부드럽고 이해하기 쉽게 써 주세요.
                   4-1. "~하면 좋아요", "~해보는 것도 좋아요", "~해보면 어때요?"처럼 공감과 제안을 담은 표현을 써 주세요.
                   4-2.단호하거나 차가운 지시형 어투(예: "데이터 분석하세요") 대신, 감성적인 설명을 써 주세요.
                   4-3.필요하다면 이모지(예: :미소짓는_얼굴:, :반짝임:)도 소량 활용해도 좋아요.
                5. 줄바꿈은 `\\\\n\\\\n`로 표시해주세요.
                6. 만약 선택한 활동 분야 (분야)가 {{활동 분야 (분야)}}라면, 실행 방법에 {{활동 분야 (분야)}}에 관련된 용어를 연관 지어서 비법을 작성해 주세요. 예를 들어 패션이라면, 패션과 관련된 전문 용어나 최신 트렌드어를 사용해 주세요.
                7. 각 실행 방법에는 하단의 예시처럼 {숫자 + . + 제목 + : + 방법}형태를 사용해 주세요.
                ...
                📌 **출력 형식 (반드시 이 JSON 형식 유지하고 비법,제목,본문,실행 방법,출처라는 항목명 유지)**
                {
                  "비법1": {
                    ...
                    "실행 방법": [
                      "1. 콘텐츠 기획 투표: \\"다음 영상에서 어떤 게 좋을까요?\\" 같은 투표를 올려보세요. (예 : '개발자 노트북 추천' vs. '직장인 생산성 앱 소개') 투표 참여가 많을수록, 영상이 올라갔을 때 조회수도 함께 증가해요.",
                      "2. 비하인드 공개: 촬영 중 마이크 오류나 코딩 실수 같은 에피소드를 짧게 공유해보세요.소소한 인간적인 모습에 댓글 반응이 활발해져요.",
                      "3. 소소한 이벤트 열기: \\"'리눅스'의 마스코트는 어떤 동물일까요? 맞히신 분 중 3분께 개발자 스티커를 보내드려요! 퀴즈형 이벤트는 댓글수를 확 끌어올리는 효과가 있어요."
                    ],
                  ...
                  "비법2": {
                    ...
                    "실행 방법": [
                      "1. 구독자 장비 세팅 리뷰: 구독자들이 보내준 데스크셋업 사진을 분석하며, 개선 팁을 전해주세요. 'OO님의 개발환경, 이렇게 하면 더 효율적!' 같은 식으로요. 시청자 참여형 콘텐츠는 댓글 유도에 탁월해요.",
                      "2. IT Q&A 받기: '코딩 공부하다 막힌 부분 있으신가요? 댓글로 질문 주세요!'라고 유도해 보세요. 댓글 수를 늘리고, 질문 기반의 후속 영상으로 자연스러운 시리즈화도 가능해요.",
                      "3. 코딩 챌린지 열기: '30줄 이하로 간단한 웹 계산기 만들기 챌린지!' 같은 챌린지를 제안하고, 제출된 결과를 영상에서 소개하면 참여율과 댓글수 모두 급상승합니다."
                    ],
                    "출처": "유튜브 [Creator Insider]"
                  },
                  "비법3": {
                    ...
                    "실행 방법": [
                      "1. 질문형 댓글 달기: '오늘 영상에서 가장 인상 깊었던 도구는?' 영상 말미에 질문을 던지거나 댓글로 직접 써두면, 자연스러운 댓글 참여를 유도할 수 있어요.",
                      "2. 고정 댓글 활용: '오늘 소개한 5개 툴 중 여러분의 최애는? 아래에 남겨주세요!' 같이 영상 핵심을 요약하며 질문을 함께 넣으면, 댓글수가 눈에 띄게 증가해요.",
                      "3. 팬 댓글 영상에 소개하기: 지난 영상에서 달린 유익한 댓글을 스크린샷으로 소개하거나 영상에 직접 활용해보세요. 팬들이 "내 댓글도 나올 수 있겠네!" 하며 더 적극적으로 참여하게 됩니다."
                    ],
                    "출처": "유튜브 [Creator Insider]"
                  }
                }
